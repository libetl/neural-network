...
export const readDatabase = async () => {
  const imagesStream = await readFileStr(
    "./train-images-idx3-ubyte",
  );
  const labels = await readFileStr(
  ...
  const imagesMetadata = Int32Array.from(
    imagesStream,
    (c) => c.codePointAt(0) || 0,
  );
  const imagesByteArray = Uint8Array.from(
    imagesStream,
    (c) => c.codePointAt(0) || 0,
  );
  ...
  for (let i = 0; i < imagesCount; i++) {
    ...
    for (let j = 0; j < width; j++) {
      for (let k = 0; k < height; k++) {
        images[i].bitmap[j][k] =
          imagesByteArray[i * width * height + j * height + k + 16];
      }
    }
  }
  return images;
