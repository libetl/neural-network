...
const activationFunctions: { [name: string]: ActivationFunction } = {
  ...
  exponentialLinearUnit: {
    apply: (number: number) =>
      number > 0 ? number : 15 * (Math.exp(number) - 1),
...
export class Network<T> {
  ...
  constructor({
    ...
    if (trainings && trainings.inputs.length) {
      const weightsAndBiases =
        this.getGradientDescent(trainings).weightsAndBiases;
    ...
    this.neurons
      ...
      .reduce(
        (acc: ProcessResult[][], layer: Neuron[]) =>
          acc.concat([layer.map((n) => {
            const rawValue = (n as HiddenLayerNeuron).process(
              acc.slice(-1)[0],
            );
            return ({
              neuron: n,
              rawValue,
              number: activationFunctions[this.activationFunction]
                .apply(rawValue),
            });
          })]),
  ...
  costSummaryOf = (trainingDataFromInput: TrainingDataset<T>): CostSummary => {
    ...
    const costs = inputs.map((input) => {
      ...
      const cost = result.reduce(
        (acc, r, i) => acc + Math.pow(r.number - expectedResult[i], 2),
        0,
      );
      return cost;
    });
  ...
  forwardAndBackwardPropagation = (
    ...
    const activationDerivative =
      activationFunctions[this.activationFunction].derivative;
    const trainingPartitions = this.partitionsOf(trainingDataFromInput);
    const partition =
      trainingPartitions[Math.floor(Math.random() * trainingPartitions.length)];
    return partition.inputs.map((input) => {
      ...
      const derivatives: Derivatives[][] = Array(resultTable.length)
        .fill(0).map((_, i): Derivatives[] =>
          Array(resultTable[i].length).fill(0).map((_, j) => ({
            outputError: 0,
            inputError: 0,
            ...
            linksDerivatives: !(this.neurons[i][j] instanceof HiddenLayerNeuron)
              ? []
              : (this.neurons[i][j] as HiddenLayerNeuron).weights.map(
                (weight) => ({
      ...
      derivatives.slice(-1)[0].forEach(
        (derivative, i) => {
          derivative.outputError = result[i] - expectedResult[i];
        },
      );

      resultTable.slice().reverse().forEach((layer, invI) => {
        const i = resultTable.length - 1 - invI;
        layer.forEach((subResult, j) => {
          const error = derivatives[i][j];
          const toCompensate = error.outputError *
            activationDerivative(subResult.rawValue);
          Object.assign(error, {
            inputError: toCompensate,
            accumulatedFromInputError: error.accumulatedFromInputError +
              toCompensate,
        ...
        layer.forEach((_, j) => {
          ...
          if (neuron instanceof HiddenLayerNeuron) {
            neuron.weights.filter((weight) => !weight.dead)
              .forEach((weight, k) => {
                const resultOfSourceNeuron = resultTable[i - 1][
                  this.neurons[i - 1].indexOf(weight.neuron)
                ].number;
                const outputError = derivative.inputError *
                  resultOfSourceNeuron;
                ...
                Object.assign(linkDerivative, {
                  neuron: weight.neuron,
                  outputError,
                  accumulatedError: linkDerivative.accumulatedError +
                    outputError,
                  numberOfAccumulatedErrors:
                    linkDerivative.numberOfAccumulatedErrors + 1,
                });
              });
          }
        });
        if (i > 1) {
          derivatives[i - 1].forEach((derivative, j) => {
            derivative.outputError = this.neurons[i].flatMap((n) => {
              const outputWeight = (n as HiddenLayerNeuron).weights.find((w) =>
                w.neuron === this.neurons[i - 1][j]
              );
              return ({
                weight: outputWeight?.number,
                inputError:
                  derivatives[i][this.neurons[i].indexOf(n)].inputError,
              });
            }).reduce(
              (acc, value) => acc + value!.weight! * value!.inputError,
              0,
            );
          });
        }
  ...
  getGradientDescent = (
      ...
      .reduce((_a, _b, round) => {
        ...
        return clone.neurons
          ...
          .map((layer: Neuron[], i: number, neurons: Neuron[][]) =>
            layer.map((neuron, j) => {
              const iteration = neurons.slice(0, i).reduce((acc, value) =>
                acc + value.length, 0) + j;
              const total = neurons.reduce((acc, value) =>
                acc + value.length, 0);
              const result = {
                  ...
                  ? (neuron as HiddenLayerNeuron).weights.map((weight) =>
                    adjust(
                      ...
                      () =>
                        clone.costSummaryOf(
                          trainingPartitions[
                            iteration % trainingPartitions.length
                          ],
                        ).average,
                    )
                  )
    ...
    const remainingCosts = clone.costSummaryOf(
      this.partitionsOf(trainingDataFromInput)[0],
    );
    return {
      weightsAndBiases,
  ...
  private partitionsOf = (
    ...
    return Array(numberOfPartitions)
      ...
      .map((_, i) => {
        let counter = 0;
        const first = (associatedInputsAndResults.length / numberOfPartitions) *
          i;
        const last = Math.min(
          first + (this.miniBatchLength || associatedInputsAndResults.length),
          (associatedInputsAndResults.length / numberOfPartitions) *
              (i + 1) - 1,
        );
        return {
